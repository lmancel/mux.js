...
const StreamTypes = require('./stream-types')
...
TransportParseStream = function() {
  ...
  parsePmt = function(payload, pmt) {
    ...
    psi.elementaryStreams.forEach(({ type, pid, esInfos }) => {
      if (type === StreamTypes.H264_STREAM_TYPE &&
          self.programMapTable.video === null) {
        self.programMapTable.video = pid
      } else if (type === StreamTypes.ADTS_STREAM_TYPE) {
      ...
      } else if (type === StreamTypes.METADATA_STREAM_TYPE) {
        // map pid to stream type for metadata streams
        self.programMapTable['timed-metadata'][pid] = type
      } else if (type === StreamTypes.PRIVATE_DATA_STREAM_TYPE) {
  ...
  this.processPes_ = function(packet, offset, result) {
    ...
    if (result.pid === this.programMapTable.video) {
      ...
      result.streamType = StreamTypes.PRIVATE_DATA_STREAM_TYPE
    } else {
    ...
    ({
      ...
      pes: function() {
        ...
        case StreamTypes.METADATA_STREAM_TYPE:
          ...
          break
        // case StreamTypes.PRIVATE_DATA_STREAM_TYPE:
