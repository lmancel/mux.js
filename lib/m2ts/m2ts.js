...
TransportParseStream = function(broadStreamDetection = false) {
  ...
  // Persist PSI in a Map in case they span across several packets
  const psiByPid = new Map()
   ...
   * @param {Buffer} payload the PMT-specific portion of an MP2T
   * packet. The first byte in this array should be the table_id
   * field.
   * @param {object} pmt the object that should be decorated with
   * fields parsed from the PMT.
   * @returns {boolean} whether the PMT was completely parsed
   * @warning The PMT may span across several packets
   */
  parsePmt = function(payload, pmt) {
    const previousPsi = psiByPid.get(pmt.pid)

    // Append to previous PSI if possible
    // if not, only create a new one if payloadUnitStartIndicator is true
    // It makes no sense otherwise to create a new PSI from an incomplete section
    if (previousPsi || pmt.payloadUnitStartIndicator) {
      const psi = previousPsi ?
        previousPsi.append(payload) :
        new ProgramSpecificInformation(payload)

        if (psi.hasBufferedEnough()) {
          psiByPid.delete(pmt.pid)
          flushPmt(psi, pmt)
          return true
        }
        psiByPid.set(pmt.pid, psi)
    }
  ...
  this.push = function(packet) {
    ...
    if (result.pid === 0) {
      ...
      result.type = 'pmt'
      const flushed = parsePsi(packet.subarray(offset), result)
    ...
    ({
      ...
      pmt: function() {
        ...
        if (programMapTable.privateData) {
          Object.entries(programMapTable.privateData)
            .forEach(([pid, { type, language }]) => {
              event.tracks.push({ type, pid: +pid, code: language })
            })
        }
