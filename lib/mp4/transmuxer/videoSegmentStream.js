 ...
 */
 module.exports = class extends Stream {
    ...
    constructor(track, options) {
        ...
        this.firstAUDIndex = -1
        this.firstKeyFrameIndex = -1
    ...
    /**
      ...
      * @param {Buffer} nalUnit.data The nalUnit bytes
    ...
    canFlush() {
      return this.lastKeyFrameIndex > this.firstKeyFrameIndex
    }

    clean() {
      const findFirstNalUnitWithType = function(array, ...types) {
        for (const [index, { nalUnitType }] of array.entries()) {
          if (types.includes(nalUnitType)) {
            return { nalUnitType, index }
          }
        }
        return {}
      }

      const TYPE_AUD_START = 'access_unit_delimiter_rbsp'
      const TYPE_KEYFRAME = 'slice_layer_without_partitioning_rbsp_idr'

      let firstAUDIndex = this.firstAUDIndex
      let firstKeyFrameIndex = this.firstKeyFrameIndex
      let lastKeyFrameIndex = this.lastKeyFrameIndex

      for (let i = Math.max(lastKeyFrameIndex, 0); i < this.nalUnits.length; i++) {
        const { nalUnitType, index } = findFirstNalUnitWithType(this.nalUnits.slice(i), TYPE_AUD_START, TYPE_KEYFRAME)
        const nextIndex = typeof index === 'number' ? i + index : i

        switch (nalUnitType) {
          case TYPE_AUD_START:
            if (firstKeyFrameIndex < 0) {
              firstAUDIndex = nextIndex
            }
            break
          case TYPE_KEYFRAME:
            if (firstAUDIndex > -1 && firstKeyFrameIndex < 0) {
              firstKeyFrameIndex = nextIndex
            }
            lastKeyFrameIndex = nextIndex
            break
        }
      ...
      if (firstAUDIndex > 0) {
        this.nalUnits = this.nalUnits.slice(firstAUDIndex)
        // We removed the "firstAUDIndex" first elements, adapt other indexes accordingly
        firstKeyFrameIndex -= firstAUDIndex
        lastKeyFrameIndex -= firstAUDIndex
      }

      // console.log('SET',
      //   'firstAUDIndex', firstAUDIndex,
      //   'firstKeyFrameIndex', firstKeyFrameIndex,
      //   'lastKeyFrameIndex', lastKeyFrameIndex,
      // )

      this.firstAUDIndex = firstAUDIndex
      this.firstKeyFrameIndex = Math.max(firstKeyFrameIndex, -1)
      this.lastKeyFrameIndex = Math.max(lastKeyFrameIndex, -1)
    }
    ...
    flush() {
      ...
      let closestAUDIndex = null

      for (let i = Math.max(this.lastKeyFrameIndex, 0); i >= 0; i--) {
        const { nalUnitType } = this.nalUnits[i]

        if (nalUnitType === 'access_unit_delimiter_rbsp') {
          closestAUDIndex = i
          break
        }
      }

      if (closestAUDIndex === null) {
        return
      }

      const nalUnits = this.nalUnits.slice(0, closestAUDIndex)
      ...
      if (this.gopsToAlignWith.length) {
        ...
        if (!alignedGops) {
          ...
          // Clear nalUnits
          this.nalUnits = this.nalUnits.slice(closestAUDIndex)
