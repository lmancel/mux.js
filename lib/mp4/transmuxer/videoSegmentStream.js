 ...
 */
 module.exports = class extends Stream {
    ...
    canFlush() {
      return this.lastAUDIndex > 0 && this.loadedNextKeyFrame
    }

    clean() {
      if (this.loadedNextKeyFrame) {
        // We already know we have buffered "enough" data
        return
      }
      // Throw away nalUnits at the start of the byte stream until
      // we find the first AUD that contains a keyframe
      let firstAUDIndex = -1
      let lastAUDIndex = -1
      let loadedKeyFrame = false
      let loadedNextKeyFrame = false

      this.nalUnits.some((nalUnit, index) => {
        switch (nalUnit.nalUnitType) {
          case 'access_unit_delimiter_rbsp':
            // keep updating first AUD index until we find the first keyframe
            if (!loadedKeyFrame) {
              firstAUDIndex = index
            }
            // keep updating last AUD index (until we find the last keyframe, see below)
            lastAUDIndex = index
            break
          case 'slice_layer_without_partitioning_rbsp_idr':
            // update last keyframe if:
            //  - first keyframe has been found
            //  - with an AUD in between
            // /!\ Order is important: update next before first
            loadedNextKeyFrame = loadedKeyFrame && lastAUDIndex > 0
            // only update first keyframe if we also have an AUD before it
            loadedKeyFrame = firstAUDIndex >= 0
            break
        }

        // return immediately upon finding a second keyframe
        return loadedNextKeyFrame
      })

      // Make sure we start with an AUD that contains a keyframe
      if (firstAUDIndex > 0) {
        this.nalUnits = this.nalUnits.slice(firstAUDIndex)
        // We removed the "firstAUDIndex" first elements, adapt lastAUDIndex accordingly
        lastAUDIndex -= firstAUDIndex
      }

      this.lastAUDIndex = lastAUDIndex
      this.loadedNextKeyFrame = loadedNextKeyFrame
    }
    ...
    flush() {
      ...
      const nalUnits = this.nalUnits.slice(0, this.lastAUDIndex)
      ...
      if (this.gopsToAlignWith.length) {
        ...
        if (!alignedGops) {
          ...
          // Clear nalUnits
          this.nalUnits = this.nalUnits.slice(this.lastAUDIndex)
