...
var timescale, startTime, compositionStartTime, getVideoTrackIds, getTrackCodec, getTracks,
 ...
 * ```
 * @param init {Buffer} the bytes of the init segment
...
/**
  ...
  *   moov > trak > tkhd
  *
  * @param {Buffer} init - The bytes of the init segment for this source
...
getTrackCodec = function(trak) {
  ...
  if (stsd) {
    ...
    codec = parseType(sampleDescriptions.subarray(4, 8))
    ...
    if (codecBox) {
      ...
      if ((/^[a-z]vc[1-9]$/i).test(codec)) {
        ...
        const codecConfigType = parseType(codecConfig.subarray(4, 8))
        ...
        if (codecConfigType === 'esds' && codecConfig.length > 20 && codecConfig[19] !== 0) {
          codec += '.' + toHexString(codecConfig[19])
          // this value is only a single digit
          codec += '.' + toHexString((codecConfig[20] >>> 2) & 0x3f).replace(/^0/, '')
        } else {
          // TODO: show a warning that we couldn't parse the codec
          // and are using the default
          codec = 'mp4a.40.2'
